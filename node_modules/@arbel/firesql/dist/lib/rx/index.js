...
firesql_1.FireSQL.prototype.rxQuery = function (sql, options) {
    utils_1.assert(typeof sql === 'string' && sql.length > 0, 'rxQuery() expects a non-empty string.');
    var ast = node_sqlparser_1.parse(sql);
...
function rxSelect(ref, ast, options) {
    ...
    if (selectOp._includeId === true) {
        idField = firesql_1.DOCUMENT_KEY_NAME;
        keepIdField = true;
    }
    ...
    var rxData = rxjs_1.combineLatest(queries.map(function (query) { return firestore_1.collectionData(query, idField); }));
