...
var SelectOperation = /** @class */ (function () {
        ...
        if (!this._includeId && Array.isArray(_ast.columns)) {
            for (var i = 0; i < _ast.columns.length; i++) {
                if (_ast.columns[i].expr.type === 'column_ref') {
                    if (_ast.columns[i].expr.column === firesql_1.DOCUMENT_KEY_NAME) {
    ...
    SelectOperation.prototype.generateQueries_ = function (ast) {
        ast = ast || this._ast;
        utils_1.assert(ast.from.length === 1, 'Only one collection at a time (no JOINs yet).');
                                                ...
                                                if (!utils_1.contains(seenDocuments, docPath)) {
                                                    ...
                                                    if (this._includeId) {
                                                        docData[typeof this._includeId === 'string'
                                                            ? this._includeId
                                                            : firesql_1.DOCUMENT_KEY_NAME] = docSnap.id;
                                                    }
    ...
    SelectOperation.prototype._buildResultEntry = function (document, aggregate, asFieldArray) {
        ...
        var resultFields = columns.reduce(function (entries, column) {
            ...
            switch (column.expr.type) {
                case 'column_ref':
                    ...
                    if (fieldName === firesql_1.DOCUMENT_KEY_NAME) {
                        idIncluded = true;
                    }
                    break;
                case 'aggr_func':
        ...
        if (this._includeId && !idIncluded) {
            resultFields.push(new AliasedField(firesql_1.DOCUMENT_KEY_NAME, typeof this._includeId === 'string'
                ? this._includeId
                : firesql_1.DOCUMENT_KEY_NAME, utils_1.safeGet(document, firesql_1.DOCUMENT_KEY_NAME)));
        }
        if (asFieldArray) {
            return resultFields;
        }
...
function aggregateDocuments(docsGroup, functions) {
    ...
    var _loop_1 = function (i) {
        ...
        functions.forEach(function (fn) {
            ...
            var value = utils_1.safeGet(doc, column);
            ...
            switch (fn.name) {
                ...
                case 'SUM':
                    if (utils_1.safeGet(skipSum, column) !== true) {
                        skipSum[column] = true;
                        if (!utils_1.contains(docsGroup.aggr.total, column)) {
                            docsGroup.aggr.total[column] = 0;
                            docsGroup.aggr.sum[column] = 0;
                        }
                        value = Number(value);
                        utils_1.assert(!Number.isNaN(value), "Can't compute aggregate function " + fn.name + "(" + column + ") because some values are not numbers.");
                    ...
                    break;
                case 'MIN':
                    utils_1.assert(isNumber || typeof value === 'string', "Aggregate function MIN(" + column + ") can only be performed on numbers or strings");
                    if (!utils_1.contains(docsGroup.aggr.min, column)) {
                        docsGroup.aggr.min[column] = value;
                    }
                    else {
                        if (!Number.isNaN(docsGroup.aggr.min[column])) {
                            // The current minimum is a number
                            utils_1.assert(isNumber, "Can't compute aggregate function MIN(" + column + ") because some values are not numbers.");
                            value = Number(value);
                        }
                    ...
                    break;
                case 'MAX':
                    utils_1.assert(isNumber || typeof value === 'string', "Aggregate function MAX(" + column + ") can only be performed on numbers or strings");
                    if (!utils_1.contains(docsGroup.aggr.max, column)) {
                        docsGroup.aggr.max[column] = value;
                    }
                    else {
                        if (!Number.isNaN(docsGroup.aggr.max[column])) {
                            // The current maximum is a number
                            utils_1.assert(isNumber, "Can't compute aggregate function MAX(" + column + ") because some values are not numbers.");
                            value = Number(value);
                        }
...
function vaidateAggrFunction(aggrFn) {
    ...
    utils_1.assert(aggrFn.args.expr.type === 'column_ref', "Unsupported type in aggregate function '" + aggrFn.name + "'.");
}
